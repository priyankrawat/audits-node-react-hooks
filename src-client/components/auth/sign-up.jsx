...
class SignUp extends React.Component {
  constructor(props) {
    ...
    super(props);
    this.state = { user: defaults, formErrors: { }, error: null };
  ...
  handleSubmit(event) {
    ...
    Object.keys(user).forEach((field) => this.validateField(field, user[field]));
    const hasErrors = Object.values(this.state.formErrors).filter((e) => e).length;
  ...
  validateField(fieldName, value) {
    const { user, formErrors } = this.state;
    const val = value !== undefined ? value : user[fieldName];
    let valid;

    switch (fieldName) {
      case 'email':
        valid = val.match(/^([\w.%+-]+)@([\w-]+\.)+([\w]{2,})$/i);
